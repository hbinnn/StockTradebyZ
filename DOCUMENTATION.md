...
# 步骤4：AI图表复评（SiliconFlow Kimi-K2.6）
python agent/siliconflow_review.py
python agent/zhipu_review.py                    # 备选：智谱 GLM-4.6V
...
│   ├── siliconflow_review.py # SiliconFlow Kimi-K2.6评审器（默认）
│   ├── gemini_review.py      # Gemini评审器
│   ├── zhipu_review.py       # 智谱GLM-4.6V评审器
│   └── prompt.md             # 交易分析提示词
├── config/                   # 配置文件
│   ├── fetch_kline.yaml      # 数据拉取配置
│   ├── rules_preselect.yaml  # B1/砖型策略参数
│   ├── siliconflow_review.yaml # SiliconFlow配置
│   ├── gemini_review.yaml    # Gemini配置
│   ├── zhipu_review.yaml    # 智谱配置
│   └── dashboard.yaml       # 看板配置
├── dashboard/                # 图表与看盘
│   ├── export_kline_charts.py  # K线图导出
│   ├── overlay_score_to_chart.py  # 评分叠加到K线图
│   ├── components/charts.py  # K线图组件
│   └── app.py               # Streamlit看盘界面
...
│   ├── select_stock.py      # B1/砖型策略实现
│   ├── Selector.py         # 选股器（Numba加速）
│   ├── pipeline_core.py     # 数据准备器
│   ├── cli.py              # 命令行入口
│   └── schemas.py          # 数据类
├── run_all.py               # 一键全流程脚本
...
详见 `pipeline/Selector.py` 中的 `B1Selector` 类，使用 `@njit` 加速计算。
...
1. **SiliconFlow Kimi-K2.6**（默认）- 配置：`config/siliconflow_review.yaml`
2. **智谱GLM-4.6V**（国产）- 配置：`config/zhipu_review.yaml`
3. **Gemini**（Google）- 配置：`config/gemini_review.yaml`

### 6.4 SiliconFlow Kimi-K2.6 配置

```yaml
# config/siliconflow_review.yaml
...
# 导出CSV格式（带股票名称）
python export_for_eastmoney.py --format csv

# 导出纯代码格式
python export_for_eastmoney.py --format plain
```
...
### Q6：评分总分超过5分
- 这是GLM返回的简单相加值而非加权值
- 已修改 prompt.md 使用加权计算公式
- 重新运行 `agent/zhipu_review.py` 可获得正确评分
