...
bailian_review.py
~~~~~~~~~~~~~~~~~~~~~
使用阿里云百炼 Qwen-3.6-Plus 对候选股票进行图表分析评分。
继承自 BaseReviewer 基础架构。

用法：
    python agent/bailian_review.py
    python agent/bailian_review.py --config config/bailian_review.yaml

配置：
    默认读取 config/bailian_review.yaml。

环境变量：
    BAILIAN_API_KEY  —— 阿里云百炼 API Key（必填）
...
_DEFAULT_CONFIG_PATH = _ROOT / "config" / "bailian_review.yaml"
...
class BailianReviewer(BaseReviewer):
    """阿里云百炼 Qwen-3.6-Plus 图表评审器。"""
    ...
    def __init__(self, config):
        ...
        if not api_key:
            print("[ERROR] 未找到环境变量 BAILIAN_API_KEY，请先设置后重试。", file=sys.stderr)
            sys.exit(1)
    ...
    def review_stock(self, code: str, day_chart: Path, prompt: str) -> dict:
        """
        调用阿里云百炼 API，对单支股票进行图表分析，返回解析后的 JSON 结果。
        支持超时和连接错误的自动重试。
        """
        headers = {
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                if "choices" not in response_body or not response_body["choices"]:
                    raise RuntimeError(f"百炼 API 返回格式异常（code={code}）：{response_body}")

                response_text = response_body["choices"][0]["message"]["content"]
                if not response_text:
                    raise RuntimeError(f"百炼 API 返回空响应，无法解析 JSON（code={code}）")
...
def main():
    parser = argparse.ArgumentParser(description="阿里云百炼 Qwen-3.6-Plus 图表复评")
    parser.add_argument(
        "--config",
